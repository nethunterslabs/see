...
class NetworkTracerHook(Hook):
    ...
    def setup_handlers(self):
        if {"start_trace_on_event", "stop_trace_on_event"} <= set(self.configuration):
            self.context.subscribe(
                self.configuration["start_trace_on_event"], self.start_trace_handler
            )
            self.logger.debug(
                "Network tracing start registered at %s event",
                self.configuration["start_trace_on_event"],
            )

            self.context.subscribe(
                self.configuration["stop_trace_on_event"], self.stop_trace_handler
            )
            self.logger.debug(
                "Network tracing stop registered at %s event",
    ...
    def start_trace_handler(self, event):
        ...
        if "trace_limit" in self.configuration:
            command.extend(("-a", "filesize:%d" % self.configuration["trace_limit"]))
...
class NetworkAnalysisHook(Hook):
    ...
    def setup_handlers(self):
        self.context.subscribe("network_tracing_started", self.network_trace_handler)
    ...
    def network_trace_handler(self, event):
        if hasattr(event, "path"):
            self.logger.debug("Event %s: new network trace %s.", event, event.path)
            self.pcap_path = event.path
        else:
            self.logging.warning("%s event received, no path specified.")

    def start_processing_handler(self, event):
        self.logger.debug("Event %s: start analysis of %s.", event, self.pcap_path)
    ...
    def stop_processing_handler(self, event):
        log_path = os.path.join(self.configuration["results_folder"], "network.log")
