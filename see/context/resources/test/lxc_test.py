...
class DomainXMLTest(unittest.TestCase):
    ...
    def test_domain_xml_filesystem(self):
        ...
        config = """<domain></domain>"""
        expected = (
            """<domain><name>foo</name><uuid>foo</uuid><devices><filesystem type="mount">"""
...
class DomainCreateTest(unittest.TestCase):
    def test_create(self):
        ...
        with mock.patch(
            "see.context.resources.lxc.open", mock.mock_open(read_data=xml), create=True
        ):
        ...
        with mock.patch(
            ...
            with mock.patch("see.context.resources.lxc.os.makedirs"):
                lxc.domain_create(
                    ...
                    {
                        "configuration": "/foo",
                        "filesystem": {"source_path": "/bar", "target_path": "/baz"},
                    },
                )
        results = hypervisor.defineXML.call_args_list[0][0][0]
        self.assertEqual(results, expected, compare(results, expected))
            ...
            with mock.patch("see.context.resources.lxc.os.makedirs"):
                lxc.domain_create(
                    ...
                    "foo",
                    {
                        "configuration": "/foo",
                        "filesystem": [
                            {"source_path": "/bar", "target_path": "/baz"},
                            {"source_path": "/dead", "target_path": "/beef"},
                        ],
                    },
                )
        results = hypervisor.defineXML.call_args_list[0][0][0]
...
class ResourcesTest(unittest.TestCase):
    ...
    def test_allocate_default(self, create_mock, libvirt_mock, network_mock):
        ...
        create_mock.assert_called_with(
            resources.hypervisor, "foo", "bar", network_name=None
        )
    ...
    def test_allocate_network(self, create_mock, libvirt_mock, network_mock):
        ...
        create_mock.assert_called_with(
            resources.hypervisor, "foo", "bar", network_name="baz"
        )
