...
class ValidAddressTest(unittest.TestCase):
    def test_valid(self):
        ...
        virnetwork.XMLDesc.side_effect = (
            lambda x: '<a><ip address="192.168.%s.1" netmask="255.255.255.0"/></a>'
            % random.randint(1, 255)
        )
        hypervisor.listNetworks.return_value = ("foo", "bar", "baz")
        hypervisor.networkLookupByName.return_value = virnetwork
        configuration = {"ipv4": "192.168.0.0", "prefix": 16, "subnet_prefix": 24}
    ...
    def test_invalid(self):
        ...
        hypervisor.listNetworks.return_value = ("foo", "bar", "baz")
        hypervisor.networkLookupByName.return_value = virnetwork
        configuration = {"ipv4": "192.168.0.1", "prefix": 16, "subnet_prefix": 24}
...
class CreateTest(unittest.TestCase):
    def test_create_too_many_attempts(self):
        ...
        configuration = {
            "configuration": "bar",
            "dynamic_address": {"ipv4": "10.0.0.0", "prefix": 16, "subnet_prefix": 24},
        }

        with mock.patch(
            "see.context.resources.network.open",
    ...
    def test_create_xml(self):
        ...
        xml = (
            """<network><forward mode="nat"/><ip address="192.168.1.1" netmask="255.255.255.0">"""
