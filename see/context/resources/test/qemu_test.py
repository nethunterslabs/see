...
class DomainXMLTest(unittest.TestCase):
    def test_domain_xml(self):
        ...
        expected = (
            """<domain><name>foo</name><uuid>foo</uuid><devices><disk device="disk" type="file">"""
    ...
    def test_domain_xml_modifies(self):
        ...
        config = (
            """<domain><name>bar</name><uuid>bar</uuid><devices><disk device="disk" type="file">"""
    ...
    def test_domain_xml_network_modifies(self):
        ...
        expected = (
            """<domain><devices><interface type="network"><source network="foo" /></interface>"""
...
<capacity>10</capacity></volume>"""
    ...
    def test_disk_xml_modifies(self):
        ...
        pool_config = """<pool><target><path>/poolpath</path></target></pool>"""
        disk_config = (
            """<volume><target><path>/path/volume.qcow2</path></target><name>bar</name>"""
            + """<capacity>10</capacity></volume>"""
        )
        expected = """<volume type="file">
  <name>foo</name>
...
<capacity>10</capacity></volume>"""
    ...
    def test_disk_cow(self):
        ...
        disk_config = """<volume><target><path>/path/volume.qcow2</path></target><capacity>10</capacity></volume>"""
        expected = (
            """<volume type="file"><name>foo</name><uuid>foo</uuid><target>"""
...
class DomainCreateTest(unittest.TestCase):
    def test_create(self):
        ...
        with mock.patch(
            "see.context.resources.qemu.open",
            mock.mock_open(read_data=xml),
            create=True,
        ):
            qemu.domain_create(
                hypervisor, "foo", {"configuration": "/foo"}, "/diskpath.qcow2"
            )
...
class PoolCreateTest(unittest.TestCase):
    ...
    def test_create(self, exists_mock, makedirs):
        """QEMU Create Pool."""
        expected = (
            """<pool type='dir'><name>foo</name><uuid>foo</uuid><target>"""
...
class DiskCloneTest(unittest.TestCase):
    ...
    def test_clone_fwdslash(self, os_mock):
        ...
        pool.XMLDesc.return_value = (
            """<pool><target><path>/pool/path</path></target></pool>"""
        )
        volume.XMLDesc.return_value = (
            """<volume><target><path>/path/volume.qcow2</path>"""
        ...
        self.assertFalse(
            "/"
            in etree.fromstring(hypervisor.storagePoolDefineXML.call_args[0][0])
            .find(".//name")
            .text
        )
    ...
    def test_clone_cow(self):
        ...
        volume.XMLDesc.return_value = (
            """<volume><target><path>/path/volume.qcow2</path></target>"""
            + """<capacity>10</capacity></volume>"""
        )
        expected = (
            """<volume type="file"><name>foo</name><uuid>foo</uuid><target>"""
    ...
    def test_clone_error(self):
        ...
        with self.assertRaises(RuntimeError) as error:
            ...
            self.assertEqual(
                str(error),
                "/foo/bar/baz.qcow2 disk must be contained in a libvirt storage pool.",
            )
...
class ResourcesTest(unittest.TestCase):
    ...
    def test_allocate_default(self, _, create_mock, libvirt_mock, network_mock):
        ...
        resources = qemu.QEMUResources(
            "foo", {"domain": "bar", "disk": {"image": "/foo/bar"}}
        )
        resources.allocate()
        libvirt_mock.open.assert_called_with("qemu:///system")
        create_mock.assert_called_with(
            resources.hypervisor, "foo", "bar", "/foo/bar", network_name=None
        )
    ...
    def test_allocate_hypervisor(self, _, create_mock, libvirt_mock, network_mock):
        ...
        network_mock.lookup.return_value = None
        resources = qemu.QEMUResources(
            "foo", {"domain": "bar", "hypervisor": "baz", "disk": {"image": "/foo/bar"}}
        )
        resources.allocate()
    ...
    @mock.patch("%s.open" % builtin_module, new_callable=mock.mock_open)
    def test_allocate_clone(
        self, _, pool_mock, disk_mock, create_mock, libvirt_mock, network_mock
    ):
    ...
    def test_allocate_network(self, _, create_mock, libvirt_mock, network_mock):
        ...
        resources = qemu.QEMUResources(
            "foo", {"domain": "bar", "network": "baz", "disk": {"image": "/foo/bar"}}
        )
        resources.allocate()
        network_mock.create.assert_called_with(resources.hypervisor, "foo", "baz")
        create_mock.assert_called_with(
            resources.hypervisor, "foo", "bar", "/foo/bar", network_name="baz"
        )
    ...
    def test_deallocate_no_creation(self, delete_mock, pool_delete_mock, network_mock):
    ...
    @mock.patch("see.context.resources.qemu.pool_delete")
    @mock.patch("see.context.resources.qemu.domain_delete")
    def test_deallocate_creation(self, delete_mock, pool_delete_mock, network_mock):
        ...
        resources = qemu.QEMUResources(
            "foo",
            {
                "domain": "bar",
                "network": {},
                "disk": {"image": "/foo/bar", "clone": {"storage_pool_path": "foo"}},
            },
        )
        resources._domain = mock.Mock()
        resources._network = mock.Mock()
