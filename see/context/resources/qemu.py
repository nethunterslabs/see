...
def disk_clone(hypervisor, identifier, storage_pool, configuration, image, logger):
    ...
    try:
        ...
        if os.path.exists(image):
            ...
            logger.info(
                "LibVirt pool does not exist, creating {} pool".format(
                    pool_path.replace("/", "_")
                )
            )
            pool = hypervisor.storagePoolDefineXML(
                BASE_POOL_CONFIG.format(pool_path.replace("/", "_"), pool_path)
            )
            ...
            raise RuntimeError("%s disk does not exist." % image)
...
class QEMUResources(resources.Resources):
    ...
    def _retrieve_pool(self):
        if "clone" in self.configuration["disk"]:
            ...
            return pool_lookup(self._hypervisor, self.provider_image)
