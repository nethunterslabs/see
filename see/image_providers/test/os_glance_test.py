...
class ImageTest(unittest.TestCase):
            ...
            "disk": {
                "image": {
                    ...
                    "provider_configuration": {
                        "path": "/foo/bar",
                        "os_auth": {"username": "dummy", "password": "dmpwd"},
                    },
                }
            }
        }
    ...
    def test_image_unavailable_target_is_dir(self, glance_mock, os_mock, _):
        ...
        resources = Resources("foo", self.config)
        expected_image_path = (
            self.config["disk"]["image"]["provider_configuration"]["path"] + "/3"
        )
        assert resources.provider_image == expected_image_path
    ...
    def test_image_unavailable_target_does_not_exist(
        self, _hom, hashlib_mock, open_mock, glance_mock, os_mock, _
    ):
        ...
        resources = Resources("foo", self.config)
        expected_image_path = (
            self.config["disk"]["image"]["provider_configuration"]["path"] + "/2"
        )
        assert resources.provider_image == expected_image_path
        glance_mock.images.data.assert_called_with("2")
        self.assertEqual(
            [mock.call("/foo/bar/2.part", "wb"), mock.call("/foo/bar/2.part", "rb")],
    ...
    def test_image_unavailable_target_is_dir_no_cached(self, glance_mock, os_mock, _):
        ...
        os_mock.path.join = os.path.join
        os_mock.path.dirname = os.path.dirname
        os_mock.path.exists.side_effect = lambda x: {
            "/foo/bar/1": False,
    ...
    def test_same_name_image_is_downloading_older_exists(
        ...
        resources = Resources("foo", self.config)
        expected_image_path = (
            self.config["disk"]["image"]["provider_configuration"]["path"] + "/1"
        )
    ...
    @mock.patch("see.image_providers.helpers.hashlib")
    @mock.patch("see.image_providers.helpers.os")
    def test_same_name_images_exist(
        self, _hom, hashlib_mock, open_mock, glance_mock, os_mock, _
    ):
        glance_mock.images.list.return_value = [self.image1, self.image2]
